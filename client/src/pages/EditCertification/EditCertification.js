...
const EditCertification = ({ inputs, title }) => {
  ...
  const sendData = () => {
    ...
    axios
      .put("/backend/api/certifications/" +clientId, data)
      .then((res , err) => {
        console.log(res);
        console.log(err);
  ...
  useEffect(() => {
    let clientId = window.location.pathname.split("/")[4];
      ...
      axios
      .get("/backend/api/certifications/"+clientId)
      .then((res) => {
        setData(res.data[0]);
  ...
  <div className="flex flex-wrap -mx-3 mb-6">
    ...
    <div className="md:w-1/2 px-3 mb-6 md:mb-0">
<br></br>
    <button type="button" onClick={(e)=>{e.preventDefault() ; sendData()}} class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Edit Certification</button>
</div>
  </div> 
